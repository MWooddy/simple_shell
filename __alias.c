...
{
  int rc;
  /*do { rc = write(...); } while(rc == -1 && errno == EINTR);*/
  do 
    { rc = write(STDOUT_FILENO, alias->key, _strlen(alias->key));}     
    while(rc == -1 && errno == EINTR);
  do { rc = write(STDOUT_FILENO, "='", 2); } while(rc == -1 && errno == EINTR);
  do { rc = write(STDOUT_FILENO, alias->val, _strlen(alias->val)); } while(rc == -1 && errno == EINTR);
  do { rc = write(STDOUT_FILENO, "'\n", 2); } while(rc == -1 && errno == EINTR);
